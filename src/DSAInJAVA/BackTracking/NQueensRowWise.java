...
public class NQueensRowWise {
    ...
    private static boolean placeNQueensHelper(int[][] board, int row) {
        if (row == board.length) {
            print1(board);
            print2(board);
            System.out.println();
        ...
        for (int i = 0; i < board.length; i++) { //going col wise
            if (isSafe(board, row, i)) { //checking for any attack
                board[row][i] = 1;
                placed = placeNQueensHelper(board, row + 1) || placed;
                board[row][i] = 0;
            }
        }
        return placed;
    ...
    private static boolean isSafe(int[][] board, int row, int col) {
        ...
        for (i = row, j = col; i >= 0 && j < board.length; i--, j++) { //diagonal down to up - only from curr col to zero col - i++
            if (board[i][j] == 1)
                return false;
        }
        return true;
